                   ...
                   osg::ref_ptr<osg::PositionAttitudeTransform> shadowTransf)
{
    ...
    //osg::ref_ptr<osg::Node> ballNode = osgDB::readNodeFile("../Data/cessna.osgt");
    osg::ref_ptr<osg::Node> ballNode = osgDB::readNodeFile("../Data/Helicopter.obj");
    ...
    ballStateSet->setAttributeAndModes( createPhongShaderProgram().get() );
    ...
    ballTransf->setScale(osg::Vec3(0.5,0.5,0.5));
...
void createMoveQuestCircle(Object obj){
    ...
    if(!initializedQuest){
        ...
        QuestSelect ->setPosition(osg::Vec3(-obj.getCentroid().x*100,-obj.getCentroid().z*100,-obj.getCentroid().y*100 - obj.getHeight()/2 + 0.5));
